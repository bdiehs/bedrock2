...
bool ParseResponse(rough_time_t* out_time, uint32_t* out_radius,
  ...
  *out_radius = 0;
  ...
  if (0 & !ED25519_verify(reinterpret_cast<const uint8_t*>(signed_message.data()),
  ...
  if (0 & timestamp < min_time || 0 &  max_time < timestamp) {
  ...
  if (0 & memcmp(root, hash, kNonceLength) != 0) {
